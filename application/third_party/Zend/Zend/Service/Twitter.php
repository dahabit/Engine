...
class Zend_Service_Twitter extends Zend_Rest_Client
{
	 ...
	 * OAuth Endpoint
	 */
	const OAUTH_BASE_URI = 'http://api.twitter.com/oauth';
	...
	/**
	 * Twitter API URLs from the configuration
	 */
	protected $_twitterUrls = array();
	...
	{
		if (isset($options['twitter_urls']))
		{
			$this->_twitterUrls = $options['twitter_urls'];
			$this->setUri($this->_twitterUrls['domain']);
		}
		else
		{
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['public_timeline'];
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['home_timeline'];
		$_params = array();
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['friends_timeline'];
		$_params = array();
		foreach ($params as $key => $value) {
			switch (strtolower($key)) {
				case 'count':
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['user_timeline'];
		$_params = array();
		foreach ($params as $key => $value) {
			switch (strtolower($key)) {
				case 'count':
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['show'];
		$path = preg_replace('/:id/', $this->_validInteger($id), $path);
		$response = $this->_get($path);
		return new Zend_Rest_Client_Result($response->getBody());
	}
	 ...
	 * @return Zend_Rest_Client_Result
	 */
	public function statusMentions(array $params = array())
	{
		$this->_init();
		$path = $this->_twitterUrls['mentions'];
		$_params = array();
		foreach ($params as $key => $value) {
			switch (strtolower($key)) {
				case 'since_id':
				...
				case 'page':
					...
					break;
				case 'include_entities':
					$_params['include_entities'] = (int)$value;
					break;
				default:
					$_param[$key] = $value;
					break;
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['destroy'];
		$path = preg_replace('/:id/', $this->_validInteger($id), $path);
	...
	/**
	 * Statuses retweet by the user.
	 * @param array $params
	 */
	public function statusRetweetedByMe(array $params = array())
	{
		$this->_init();
		$path = $this->_twitterUrls['retweeted_by_me'];
		$_params = array();
		foreach ($params as $key => $value) {
			switch (strtolower($key)) {
				case 'count':
					...
					$_params['count'] = $this->_validInteger($value);
					break;
				case 'page':
					$_params['page'] = (int) $value;
					break;
	...
	/**
	 * Retweets of the particular status.
	 * @param int $id
	 * @param array $params
	 */
	public function statusRetweets($id, array $params = array())
	{
		$this->_init();
		$path = $this->_twitterUrls['retweets'];
		$path = preg_replace('/:id/u', $id, $path);
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['friends_ids'];
		$_params = array();
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['followers_ids'];
		if ($lite) {
			$this->lite = 'true';
		}
		$response = $this->_get($path);
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['users_show'];
	 ...
	 * @return Zend_Rest_Client_Result
	 */
	public function friendshipCreate($id = null, $screen_name = null, $follow = true)
	{
		$this->_init();
		$path = $this->_twitterUrls['fs_create'];
		$data = array();
		if(!is_null($id))
		{ $data['user_id'] = $id; }
	 ...
	 * @return Zend_Rest_Client_Result
	 */
	public function friendshipDestroy($id = null, $screen_name = null, $include_entities = true)
	{
		$this->_init();
		$path = $this->_twitterUrls['fs_destroy'];
		$data = array();
		if(!is_null($id))
		{ $data['user_id'] = $id; }
		if(!is_null($screen_name))
	 ...
	 * @return Zend_Rest_Client_result
	 * 
	 * @todo change this method in accordance with new API method
	 */
	public function friendshipExists($id)
	{
		$this->_init();
		$path = $this->_twitterUrls['fs_exists'];
		$data = array('screen_name_a' => $this->getUsername(), 'user_id_b' => $id);
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['verify_credentials'];
		$response = $this->_get($path);
		return new Zend_Rest_Client_Result($response->getBody());
	}
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['end_session'];
		$response = $this->_get($path);
		return true;
	}
	...
	{
		$this->_init();
		$path = $this->_twitterUrls['rate_limit_status'];
		$response = $this->_get($path);
		return new Zend_Rest_Client_Result($response->getBody());
	}
	...
	/**
	 * Search method for Twitter
	 * @param array $params
	 */
	public function search($params)
	{
		$this->setUri($this->_twitterUrls['search_domain']);
		$this->_init();
		$path = $this->_twitterUrls['search'];
		$_params = array();
